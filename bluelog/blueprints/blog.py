...
def show_post(post_id):
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['BLUE_COMMENT_PER_PAGE']
    ...
    if True: # current_user.is_authenticated:
        ...
        form.author.data = "current_user" # current_user.name
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        if True: #current_user.is_authenticated:
            flask('Comment published.', 'success')
        else:
            flask('Thanks, your comment will be published after reviewed.', 'info')
